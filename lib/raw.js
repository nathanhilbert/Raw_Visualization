...
!function (exports){
  ...
  raw.model = function () { 
    ...
    model.getOptions = function(){
      var returnset = [];
      jQuery.each(dimensions["_"], function(i, v){
        returnset.push({
          'key': i,
          'value': v.value,
        });
    ...
    model.setOptions = function(optSet){
      jQuery.each(optSet, function(i, v){
        dimensions["_"][v['key']].value = v['value'];
      });
    };
  ...
  raw.chart = function (id) { 
    ...
    chart.getOptions = function(){
      //don't know whats up with this underscore
      var returnset = {};
      jQuery.each(options["_"], function(i, v){
        if (v.type() == "color"){
          var values = {};
          v.domain().forEach(function(subi, subv){
            values[subi] = v.value(subi);
          });
          returnset[v.title()] = {
            'type': v.type(),
            'values': values
    ...
    chart.setOptions = function(optSet, scope){
      jQuery.each(options["_"], function(i, v){
        if (v.type() == 'color'){
          //I think the keys and the values are a mapped field
          var mappedcolors = d3.map(optSet[v.title()]['values']);
          v.domain(mappedcolors.keys());
          scope.$apply();
          v.value.range(mappedcolors.values());
        }
        else{
          v.value = optSet[v.title()].value;
        }
      });
      //emit update
    };
